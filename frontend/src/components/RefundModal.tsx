...
import { refundService, REFUND_METHODS, type CreateRefundPayload } from '../services/refundService';
...
export function RefundModal({ label, maxAmount, orderId, sessionId, onSuccess, onClose }: Props) {
  ...
  async function handleSubmit() {
    ...
    if (parsedAmount > maxAmount) { toast.error(`Cannot exceed ${fmt(maxAmount)}`); return; }
    if (!reason.trim())           { toast.error('Reason is required'); return; }

    setLoading(true);
    try {
      const payload: CreateRefundPayload = { amount: parsedAmount, reason: reason.trim(), refundMethod: method };
      if (orderId)   payload.orderId   = orderId;
      if (sessionId) payload.sessionId = sessionId;

      await refundService.createRefund(payload);
      toast.success(`Refund of ${fmt(parsedAmount)} issued`);
      onSuccess();
      onClose();
    } catch (err) {
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          >
            ...
            {loading ? 'Processing…' : `Refund ${amountValid ? fmt(parsedAmount) : ''}`}
          </button>
        </div>
      </div>
    </div>
