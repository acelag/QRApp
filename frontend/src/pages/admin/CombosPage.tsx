...
export function CombosPanel() {
  ...
  function openCreate() {
    setEditing(null); setForm(EMPTY_FORM()); setPreview(''); setItemSearch(''); setShowModal(true);
  }

  function openEdit(combo: Combo) {
    setEditing(combo);
    setForm({ name: combo.name, description: combo.description ?? '', price: combo.price, image: combo.image ?? '', active: combo.active, sortOrder: combo.sortOrder, items: combo.items.map((i) => ({ menuItemId: i.menuItemId, quantity: i.quantity })) });
    setPreview(combo.image ?? ''); setItemSearch(''); setShowModal(true);
  }
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setForm((p) => ({ ...p, image: url })); toast.success('Image uploaded');
    } catch { toast.error('Image upload failed'); setPreview(form.image ?? ''); }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const payload: ComboPayload = { ...form, description: form.description?.trim() || undefined, image: form.image?.trim() || undefined, items: form.items.filter((i) => i.quantity > 0) };
      const saved = editing ? await comboService.updateCombo(editing.id, payload) : await comboService.createCombo(payload);
      setCombos((prev) => editing ? prev.map((c) => (c.id === saved.id ? saved : c)) : [...prev, saved]);
      setShowModal(false); toast.success(editing ? 'Combo updated' : 'Combo created');
  ...
  async function handleDelete(id: string) {
    try {
      ...
      setCombos((prev) => prev.filter((c) => c.id !== id)); setDeleteId(null); toast.success('Combo deleted');
    } catch { toast.error('Delete failed'); }
  }

  function getQty(menuItemId: string) { return form.items.find((i) => i.menuItemId === menuItemId)?.quantity ?? 0; }
  function setQty(menuItemId: string, qty: number) {
    setForm((prev) => { const filtered = prev.items.filter((i) => i.menuItemId !== menuItemId); if (qty <= 0) return { ...prev, items: filtered }; return { ...prev, items: [...filtered, { menuItemId, quantity: qty }] }; });
  }

  const filteredMenuItems = itemSearch.trim() ? menuItems.filter((i) => i.name.toLowerCase().includes(itemSearch.toLowerCase())) : menuItems;

  return (
    <div className="px-3 sm:px-4 lg:px-6 py-4">
      {/* Toolbar */}
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-500">{loading ? '' : `${combos.length} combo${combos.length !== 1 ? 's' : ''}`}</p>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(combo)} className="p-1.5 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors"><Pencil size={14} /></button>
                    <button onClick={() => setDeleteId(combo.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))}
                  rows={2} placeholder="What makes this deal special?"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min={0} step={0.01} value={form.price || ''} onChange={(e) => setForm((p) => ({ ...p, price: parseFloat(e.target.value) || 0 }))}
                    placeholder="0.00" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-orange-300" />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="w-24 h-24 rounded-2xl bg-gray-100 overflow-hidden flex items-center justify-center shrink-0 border-2 border-dashed border-gray-200">
                    {uploading ? <Loader2 size={28} className="text-orange-400 animate-spin" /> : preview ? <img src={preview} alt="preview" className="w-full h-full object-cover" /> : <Package size={28} className="text-gray-300" />}
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                      ...
                      <ImagePlus size={16} />{preview ? 'Change photo' : 'Browse photo'}
                    </button>
                    {preview && !uploading && (
                      <button type="button" onClick={() => { setPreview(''); setForm((p) => ({ ...p, image: '' })); }} className="text-xs text-red-400 hover:text-red-500 text-left">Remove photo</button>
                    )}
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setForm((p) => ({ ...p, active: !p.active }))}
                  ...
                  {form.active ? <ToggleRight size={22} /> : <ToggleLeft size={22} />}{form.active ? 'Yes' : 'No'}
                </button>
              </div>
              <div>
                ...
                <input type="text" value={itemSearch} onChange={(e) => setItemSearch(e.target.value)} placeholder="Search menu items…"
                  ...
                  {filteredMenuItems.length === 0 ? (
                    ...
                    return (
                      <div key={item.id} className="flex items-center justify-between px-3 py-2 hover:bg-gray-50">
                        <div><p className="text-sm font-medium text-gray-800">{item.name}</p><p className="text-xs text-gray-400">{fmt(item.price)}</p></div>
                        <div className="flex items-center gap-1.5">
                          {qty > 0 ? (
                            <>
                              <button onClick={() => setQty(item.id, qty - 1)} className="w-6 h-6 flex items-center justify-center rounded-full bg-orange-100 text-orange-600 hover:bg-orange-200 transition-colors"><Minus size={12} /></button>
                              <span className="w-5 text-center text-sm font-semibold text-gray-800">{qty}</span>
                              <button onClick={() => setQty(item.id, qty + 1)} className="w-6 h-6 flex items-center justify-center rounded-full bg-orange-100 text-orange-600 hover:bg-orange-200 transition-colors"><Plus size={12} /></button>
                            </>
                          ) : (
                            <button onClick={() => setQty(item.id, 1)} className="text-xs bg-orange-500 text-white px-3 py-1 rounded-full hover:bg-orange-600 transition-colors">Add</button>
                          )}
                        </div>
                      </div>
                    );
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto mt-14 md:mt-0">
        <AdminHeader title="Combo Deals" backTo="/admin" />
        <CombosPanel />
      </main>
    </div>
  );
