...
export function MenuSetupPanel() {
  ...
  function startEditCat(cat: Category) { setEditingCat(cat); setEditingCatName(cat.name); }
  ...
  async function addTag() {
    ...
    try {
      ...
      setTags((prev) => [...prev, t]);
      setNewTagLabel(''); setNewTagEmoji('🏷️'); setNewTagCategory('label');
      toast.success('Tag added');
    } catch { toast.error('Failed to add tag'); } finally { setSavingTag(false); }
  }
  ...
  async function saveEditTag() {
    ...
    } catch { toast.error('Failed to update tag'); } finally { setSavingTag(false); }
  }
    ...
    <div className="px-3 sm:px-4 lg:px-6 py-4 space-y-4 max-w-5xl">
              ...
              <div key={cat.id} className="flex items-center gap-1 bg-orange-50 border border-orange-200 rounded-full px-2 py-0.5">
                {editingCat?.id === cat.id ? (
                  ...
                  <>
                    ...
                    {catItems.length > 0 && (
                      <>
                        <button onClick={() => handleBulkToggle(cat, true)} disabled={isBusy || allOn} title="Mark all available"
                        ...
                        <button onClick={() => handleBulkToggle(cat, false)} disabled={isBusy || allOff} title="Mark all unavailable"
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                ...
                {catTags.map((tag) => (
                  <div key={tag.id} className={`flex items-center gap-1 border rounded-full px-2 py-0.5 ${tagPillCls(tag.category)} border-current/20`}>
                    {editingTag?.id === tag.id ? (
                      <>
                        <input value={editingTag.emoji} onChange={(e) => setEditingTag({ ...editingTag, emoji: e.target.value })}
                          className="w-8 text-center text-sm bg-transparent outline-none" maxLength={4} title="Emoji" />
                        <input autoFocus value={editingTag.label} onChange={(e) => setEditingTag({ ...editingTag, label: e.target.value })}
                        ...
                        <select value={editingTag.category} onChange={(e) => setEditingTag({ ...editingTag, category: e.target.value as TagCategory })}
                      ...
                      <>
                        ...
                        <button onClick={() => setEditingTag({ id: tag.id, label: tag.label, emoji: tag.emoji, category: tag.category })}
                          className="opacity-60 hover:opacity-100 transition-opacity shrink-0" title="Edit"><Pencil size={12} /></button>
                        <button onClick={() => deleteTag(tag)} className="opacity-40 hover:text-red-500 hover:opacity-100 transition-opacity shrink-0" title="Delete"><Trash2 size={12} /></button>
                      </>
                    )}
                  </div>
                ))}
        ...
        <div className="flex gap-2 mt-2 pt-2 border-t border-gray-100">
          <input value={newTagEmoji} onChange={(e) => setNewTagEmoji(e.target.value)} placeholder="🏷️" maxLength={4}
            className="w-12 text-center border border-gray-200 rounded-lg px-2 py-1.5 text-sm outline-none focus:ring-1 focus:ring-orange-300" title="Emoji" />
          <input value={newTagLabel} onChange={(e) => setNewTagLabel(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addTag()}
            placeholder="New tag name" className="flex-1 border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-orange-300" />
          ...
          <button onClick={addTag} disabled={savingTag || !newTagLabel.trim()}
            className="bg-orange-500 text-white px-3 py-1.5 rounded-lg text-sm hover:bg-orange-600 transition-colors disabled:opacity-50">Add</button>
        </div>
      </div>
    </div>
  );
